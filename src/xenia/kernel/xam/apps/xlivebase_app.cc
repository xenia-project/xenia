...
X_RESULT XLiveBaseApp::DispatchMessageSync(uint32_t message,
  ...
  switch (message) {
    ...
    case 0x00058020: {
      ...
      XELOGD("CXLiveFriends::Enumerate(%.8X, %.8X) unimplemented",
             buffer_ptr, buffer_length);
      return X_STATUS_UNSUCCESSFUL;
    }
    case 0x00058023: {
      XELOGD("CXLiveMessaging::XMessageGameInviteGetAcceptedInfo(%.8X, %.8X) unimplemented", buffer_ptr,
             buffer_length);
      return X_STATUS_UNSUCCESSFUL;
    }
	...
	case 0x00058006: {		
		...
		XELOGD("CXLiveLogon::GetNatType(%.8X, %8X)", buffer_ptr,
		buffer_length);
	...
	case 0x00058007: {
		/* 
			Occurs if title calls XOnlineGetServiceInfo, expects dwServiceId and pServiceInfo.
			pServiceInfo should contain pointer to XONLINE_SERVICE_INFO structure.
		*/
