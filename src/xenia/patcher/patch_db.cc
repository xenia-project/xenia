...
PatchFileEntry PatchDB::ReadPatchFile(const std::filesystem::path& file_path) {
  ...
  patch_file.title_name = *patch_toml_fields->get_as<std::string>("title_name");
  ...
  for (auto patch_table_entry : *patch_table) {
    ...
    patch.patch_name = *patch_table_entry->get_as<std::string>("name");
    patch.patch_desc = *patch_table_entry->get_as<std::string>("desc");
    patch.patch_author = *patch_table_entry->get_as<std::string>("author");
    patch.is_enabled = *patch_table_entry->get_as<bool>("is_enabled");
    ...
    for (const auto& patch_data_type : patch_data_types_size_) {
      ...
      if (!success) {
        XELOGE("PatchDB: Cannot read patch {}", patch.patch_name);
        break;
      }
    }
