...
void* AutoreleasePoolTracker::Push(const char* location) {
  ...
  XELOGI(
      "[AutoreleasePool] PUSH: depth={}, location='{}', ptr={:x}, thread='{}'",
...
void AutoreleasePoolTracker::Pop(void* pool, const char* location) {
  ...
  if (!pool_stack_.empty()) {
    ...
    if (top.pool != pool) {
      XELOGE(
          "[AutoreleasePool] ERROR: Pool mismatch! Expected {:x} but got {:x} "
          "at {}",
      ...
      for (auto it = pool_stack_.rbegin(); it != pool_stack_.rend(); ++it) {
        if (it->pool == pool) {
          XELOGE(
              "[AutoreleasePool] Pool was pushed at '{}' but not in expected "
              "order",
              it->location);
          found = true;
          break;
      ...
      XELOGI(
          "[AutoreleasePool] POP: depth={}, location='{}', ptr={:x}, "
          "duration={}us, thread='{}'",
          pool_depth_, location, reinterpret_cast<uintptr_t>(pool), duration_us,
          thread_name);
    ...
    XELOGE(
        "[AutoreleasePool] ERROR: Popping pool {:x} but stack is empty at {}",
        reinterpret_cast<uintptr_t>(pool), location ? location : "unknown");
  }

  // Actually pop the pool
  XELOGI("[AutoreleasePool] About to call objc_autoreleasePoolPop({:x})",
         reinterpret_cast<uintptr_t>(pool));
  objc_autoreleasePoolPop(pool);
  XELOGI("[AutoreleasePool] objc_autoreleasePoolPop returned");
}
...
void AutoreleasePoolTracker::CheckLeaks() {
  ...
  if (!pool_stack_.empty()) {
    ...
    XELOGE(
        "[AutoreleasePool] LEAK DETECTED: {} pools not drained on thread '{}'",
