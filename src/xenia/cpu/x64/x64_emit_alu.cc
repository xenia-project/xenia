...
XEEMITTER(addx,         0x7C000214, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RD <- (RA) + (RB)
...
XEEMITTER(addcx,        0x7C000014, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(addex,        0x7C000114, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(addi,         0x38000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if RA = 0 then
  //   RT <- EXTS(SI)
  // else
  //   RT <- (RA) + EXTS(SI)
...
XEEMITTER(addic,        0x30000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RT <- (RA) + EXTS(SI)
...
XEEMITTER(addicx,       0x34000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(addis,        0x3C000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if RA = 0 then
  //   RT <- EXTS(SI) || i16.0
  // else
  //   RT <- (RA) + EXTS(SI) || i16.0
...
XEEMITTER(addmex,       0x7C0001D4, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(addzex,       0x7C000194, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RT <- (RA) + CA
...
XEEMITTER(divdx,        0x7C0003D2, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(divdux,       0x7C000392, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

// XEEMITTER(divwx,        0x7C0003D6, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
...
// XEEMITTER(divwux,       0x7C000396, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
//   // dividend[0:31] <- (RA)[32:63]
//   // divisor[0:31] <- (RB)[32:63]
//   // if divisor = 0 then
//   //   if OE = 1 then
...
XEEMITTER(mulhdx,       0x7C000092, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(mulhdux,      0x7C000012, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(mulhwx,       0x7C000096, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(mulhwux,      0x7C000016, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(mulldx,       0x7C0001D2, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(mulli,        0x1C000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
...
XEEMITTER(mullwx,       0x7C0001D6, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
...
// XEEMITTER(negx,         0x7C0000D0, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
//   // RT <- ¬(RA) + 1
...
XEEMITTER(subfx,        0x7C000050, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RT <- ¬(RA) + (RB) + 1
...
XEEMITTER(subfcx,       0x7C000010, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

// XEEMITTER(subficx,      0x20000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
//   // RT <- ¬(RA) + EXTS(SI) + 1
...
XEEMITTER(subfex,       0x7C000110, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RT <- ¬(RA) + (RB) + CA
...
XEEMITTER(subfmex,      0x7C0001D0, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(subfzex,      0x7C000190, XO )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}
...
XEEMITTER(cmp,          0x7C000000, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if L = 0 then
  //   a <- EXTS((RA)[32:63])
  //   b <- EXTS((RB)[32:63])
  // else
...
XEEMITTER(cmpi,         0x2C000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if L = 0 then
  //   a <- EXTS((RA)[32:63])
  // else
  //   a <- (RA)
...
XEEMITTER(cmpl,         0x7C000040, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if L = 0 then
  //   a <- i32.0 || (RA)[32:63]
  //   b <- i32.0 || (RB)[32:63]
  // else
...
XEEMITTER(cmpli,        0x28000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // if L = 0 then
  //   a <- i32.0 || (RA)[32:63]
  // else
  //   a <- (RA)
...
XEEMITTER(andx,         0x7C000038, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) & (RB)
...
XEEMITTER(andcx,        0x7C000078, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) & ¬(RB)
...
XEEMITTER(andix,        0x70000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) & (i48.0 || UI)
...
XEEMITTER(andisx,       0x74000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
...
XEEMITTER(cntlzdx,      0x7C000074, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

// XEEMITTER(cntlzwx,      0x7C000034, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
//   // n <- 32
//   // do while n < 64
...
XEEMITTER(eqvx,         0x7C000238, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(extsbx,       0x7C000774, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // s <- (RS)[56]
  // RA[56:63] <- (RS)[56:63]
  // RA[0:55] <- i56.s
...
XEEMITTER(extshx,       0x7C000734, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(extswx,       0x7C0007B4, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(nandx,        0x7C0003B8, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(norx,         0x7C0000F8, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- ¬((RS) | (RB))
...
XEEMITTER(orx,          0x7C000378, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) | (RB)
...
XEEMITTER(orcx,         0x7C000338, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(ori,          0x60000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) | (i48.0 || UI)
...
XEEMITTER(oris,         0x64000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) | (i32.0 || UI || i16.0)
...
XEEMITTER(xorx,         0x7C000278, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) XOR (RB)
...
XEEMITTER(xori,         0x68000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // RA <- (RS) XOR (i48.0 || UI)
...
XEEMITTER(xoris,        0x6C000000, D  )(X64Emitter& e, jit_function_t f, InstrData& i) {
...
XEEMITTER(rldclx,       0x78000010, MDS)(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(rldcrx,       0x78000012, MDS)(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(rldicx,       0x78000008, MD )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(rldiclx,      0x78000000, MD )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // n <- sh[5] || sh[0:4]
  // r <- ROTL64((RS), n)
  // b <- mb[5] || mb[0:4]
  // m <- MASK(b, 63)
...
XEEMITTER(rldicrx,      0x78000004, MD )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(rldimix,      0x7800000C, MD )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(rlwimix,      0x50000000, M  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // n <- SH
  // r <- ROTL32((RS)[32:63], n)
  // m <- MASK(MB+32, ME+32)
  // RA <- r&m | (RA)&¬m
...
XEEMITTER(rlwinmx,      0x54000000, M  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // n <- SH
  // r <- ROTL32((RS)[32:63], n)
  // m <- MASK(MB+32, ME+32)
  // RA <- r & m
...
XEEMITTER(rlwnmx,       0x5C000000, M  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}
...
XEEMITTER(sldx,         0x7C000036, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(slwx,         0x7C000030, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // n <- (RB)[59:63]
  // r <- ROTL32((RS)[32:63], n)
  // if (RB)[58] = 0 then
  //   m <- MASK(32, 63-n)
...
XEEMITTER(sradx,        0x7C000634, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(sradix,       0x7C000674, XS )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(srawx,        0x7C000630, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(srawix,       0x7C000670, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  // n <- SH
...
XEEMITTER(srdx,         0x7C000436, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}

XEEMITTER(srwx,         0x7C000430, X  )(X64Emitter& e, jit_function_t f, InstrData& i) {
  XEINSTRNOTIMPLEMENTED();
  return 1;
}
