...
X64Emitter::X64Emitter(xe_memory_ref memory, jit_context_t context) {
  memory_ = memory;
  context_ = context;
...
jit_context_t X64Emitter::context() {
  return context_;
}
...
void X64Emitter::GenerateBasicBlock(FunctionBlock* block) {
  ...
  for (uint32_t ia = block->start_address; ia <= block->end_address; ia += 4) {
    ...
    typedef int (*InstrEmitter)(X64Emitter& g, jit_function_t f,
                                InstrData& i);
