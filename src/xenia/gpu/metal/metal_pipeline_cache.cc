...
void MetalPipelineCache::AugmentTextureBindings(MetalShader* shader, 
  ...
  // CRITICAL FIX: When MSC reports 0 slots but we detect valid Xbox 360 textures,
  // we need to proactively create synthetic bindings for those textures.
  // This is essential for Xbox 360 vertex fetch patterns where MSC's automatic
  // layout mode doesn't detect texture usage in vertex shaders properly.
  
  bool msc_detected_textures = !msc_texture_slots.empty();
  bool shader_has_bindings = !shader_bindings.empty();
  
  // If MSC detected texture slots, process them normally
  if (msc_detected_textures) {
    XELOGI("Metal: Processing MSC-detected texture slots");
    ...
    // MSC didn't detect any texture slots - scan Xbox 360 fetch constants proactively
    XELOGI("Metal: MSC detected 0 texture slots - scanning Xbox 360 fetch constants");
    
    // Scan all possible Xbox 360 fetch constants (0-31) to find valid textures
    std::vector<uint32_t> detected_fetch_constants;
    for (uint32_t fc = 0; fc < 32; fc++) {
      xenos::xe_gpu_texture_fetch_t fetch = register_file_->GetTextureFetch(fc);
      if (fetch.base_address != 0) {
        detected_fetch_constants.push_back(fc);
        XELOGI("  Found valid texture at fetch constant {} (base_address=0x{:08X})", 
               fc, fetch.base_address);
      }
    }
    
    // Create synthetic bindings for all detected textures
    for (uint32_t fc : detected_fetch_constants) {
      // Check if we already have a binding for this fetch constant
      bool found = false;
      for (const auto& binding : shader_bindings) {
        if (binding.fetch_constant == fc) {
          found = true;
          XELOGI("  Fetch constant {} already has binding", fc);
          break;
        }
      }
      
      if (!found) {
        // Get the actual fetch constant data
        xenos::xe_gpu_texture_fetch_t fetch = register_file_->GetTextureFetch(fc);
        
        // Create synthetic binding with real fetch constant data
        Shader::TextureBinding new_binding = {};
        new_binding.fetch_constant = fc;
        new_binding.binding_index = shader_bindings.size();
        ...
        shader_bindings.push_back(new_binding);
        XELOGI("  Added synthetic binding for fetch constant {} (base_address=0x{:08X}, dimension={})", 
               fc, fetch.base_address, static_cast<uint32_t>(fetch_instr.dimension));
      }
    }
    
    XELOGI("Metal: Proactive scan found {} valid textures, created {} synthetic bindings",
           detected_fetch_constants.size(), detected_fetch_constants.size() - (shader_bindings.size() - detected_fetch_constants.size()));
  }
  ...
  // CRITICAL: Update the texture slots to match augmented bindings
  // This ensures MSC's runtime system knows about all the texture slots we created
  if (!shader_bindings.empty()) {
    ...
    XELOGI("Metal: Updated texture slots to match {} synthetic bindings", updated_slots.size());
  }
}
