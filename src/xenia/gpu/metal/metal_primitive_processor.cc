...
bool MetalPrimitiveProcessor::InitializeBuiltinIndexBuffer(
  ...
  builtin_index_buffer_ = device->newBuffer(size_bytes, 
  ...
  builtin_index_buffer_->setLabel(
      NS::String::string("Xenia Built-in Index Buffer", 
...
void* MetalPrimitiveProcessor::RequestHostConvertedIndexBufferForCurrentFrame(
  ...
  size_t element_size = format == xenos::IndexFormat::kInt16 
                        ? sizeof(uint16_t) 
                        : sizeof(uint32_t);
  ...
  if (!chosen_buffer) {
    ...
    MTL::Buffer* new_buffer = device->newBuffer(
        allocation_size, MTL::ResourceStorageModeShared);
    ...
    snprintf(label, sizeof(label), 
             "Xenia Converted Index Buffer (%zu bytes)", allocation_size);
  ...
  if (coalign_for_simd) {
    ptrdiff_t offset = GetSimdCoalignmentOffset(cpu_buffer, 
                                                coalignment_original_address);
