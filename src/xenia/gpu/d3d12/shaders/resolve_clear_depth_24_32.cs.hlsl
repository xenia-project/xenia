...
void main(uint3 xe_thread_id : SV_DispatchThreadID) {
  ...
  uint address_int4s =
      XeEdramOffsetInts(
          ...
          kXenosMsaaSamples_1X, true, 0u)
      >> 2u;
  // Guest 24-bit depth/stencil.
  xe_resolve_dest[address_int4s] = xe_resolve_clear_value.xxxx;
  xe_resolve_dest[address_int4s + 1u] = xe_resolve_clear_value.xxxx;
  // Host 32-bit depth in the second 10 MB.
  xe_resolve_dest[address_int4s + 2048u * 320u] =
      xe_resolve_clear_value.yyyy;
  xe_resolve_dest[address_int4s + (2048u * 320u + 1u)] =
      xe_resolve_clear_value.yyyy;
}
