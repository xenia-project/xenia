...
bool TextureCache::LoadTextureData(Texture* texture) {
  ...
  if (!base_in_sync) {
    if (!shared_memory_.RequestRange(
            texture->key.base_page << 12, texture->base_size,
  ...
  if (!mips_in_sync) {
    if (!shared_memory_.RequestRange(
            texture->key.mip_page << 12, texture->mip_size,
  ...
  // Array slices are within mips (similar to the guest addressing). All
  // subresources, including the last one, have both offset and size aligned to
  // D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT for simplicity of both array slice
  // addressing and writing to the last row.
  UINT64 copy_buffer_size = 0;
  D3D12_PLACED_SUBRESOURCE_FOOTPRINT
  host_unpacked_slice_0_layouts[D3D12_REQ_MIP_LEVELS];
  UINT64 host_unpacked_slice_sizes[D3D12_REQ_MIP_LEVELS];
  D3D12_PLACED_SUBRESOURCE_FOOTPRINT host_packed_slice_0_layout = {};
  UINT64 host_packed_slice_size = 0;
  UINT64 host_packed_mips_offset = 0;
  // If need to load both base and mips, and both are in the packed mip tail,
  // but base and mip addresses are different, host_packed_slice_0_layout and
  // host_packed_slice_size are for the base level, and mips are at
  // host_packed_slice_0_layout.Offset + host_packed_mips_offset.
  {
    ...
    uint32_t copy_block_width;
    uint32_t copy_block_height;
    uint32_t copy_bytes_per_block;
    ui::d3d12::util::GetFormatCopyInfo(
        GetDXGIResourceFormat(guest_format, width, height), 0, copy_format,
        copy_block_width, copy_block_height, copy_bytes_per_block);
    if (mip_first < mip_packed) {
      uint32_t width_scaled = width * texture_resolution_scale;
      uint32_t height_scaled = height * texture_resolution_scale;
      uint32_t host_layout_mip_count =
          std::min(mip_packed, mip_last + uint32_t(1)) - mip_first;
      for (uint32_t i = 0; i < host_layout_mip_count; ++i) {
        D3D12_PLACED_SUBRESOURCE_FOOTPRINT& mip_layout =
            host_unpacked_slice_0_layouts[i];
        mip_layout.Offset = copy_buffer_size;
        mip_layout.Footprint.Format = copy_format;
        uint32_t mip = mip_first + i;
        uint32_t mip_copy_width_blocks =
            (std::max(width_scaled >> mip, uint32_t(1)) +
             (copy_block_width - uint32_t(1))) /
            copy_block_width;
        uint32_t mip_copy_height_blocks =
            (std::max(height_scaled >> mip, uint32_t(1)) +
             (copy_block_height - uint32_t(1))) /
            copy_block_height;
        mip_layout.Footprint.Width = mip_copy_width_blocks * copy_block_width;
        mip_layout.Footprint.Height =
            mip_copy_height_blocks * copy_block_height;
        mip_layout.Footprint.Depth = std::max(depth >> mip, uint32_t(1));
        mip_layout.Footprint.RowPitch =
            xe::align(xe::round_up(mip_copy_width_blocks,
                                   load_shader_info.host_x_blocks_per_thread) *
                          copy_bytes_per_block,
                      uint32_t(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT));
        UINT64 copy_slice_size =
            xe::align(UINT64(mip_layout.Footprint.RowPitch) *
                          mip_copy_height_blocks * mip_layout.Footprint.Depth,
                      UINT64(D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT));
        host_unpacked_slice_sizes[i] = copy_slice_size;
        copy_buffer_size += copy_slice_size * slice_count;
      }
    }
    if (mip_last >= mip_packed) {
      host_packed_slice_0_layout.Offset = copy_buffer_size;
      host_packed_slice_0_layout.Footprint.Format = copy_format;
      uint32_t mip_packed_copy_width_blocks =
          (mip_packed_width * texture_resolution_scale +
           (copy_block_width - uint32_t(1))) /
          copy_block_width;
      uint32_t mip_packed_copy_height_blocks =
          (mip_packed_height * texture_resolution_scale +
           (copy_block_height - uint32_t(1))) /
          copy_block_height;
      host_packed_slice_0_layout.Footprint.Width =
          mip_packed_copy_width_blocks * copy_block_width;
      host_packed_slice_0_layout.Footprint.Height =
          mip_packed_copy_height_blocks * copy_block_height;
      host_packed_slice_0_layout.Footprint.Depth = mip_packed_depth;
      host_packed_slice_0_layout.Footprint.RowPitch =
          xe::align(xe::round_up(mip_packed_copy_width_blocks,
                                 load_shader_info.host_x_blocks_per_thread) *
                        copy_bytes_per_block,
                    uint32_t(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT));
      host_packed_slice_size =
          xe::align(UINT64(host_packed_slice_0_layout.Footprint.RowPitch) *
                        mip_packed_copy_height_blocks * mip_packed_depth,
                    UINT64(D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT));
      UINT64 host_packed_total_size = host_packed_slice_size * slice_count;
      copy_buffer_size += host_packed_total_size;
      if (mip_packed == 0 && mip_first == 0 && mip_last != 0 &&
        ...
        host_packed_mips_offset = host_packed_total_size;
        copy_buffer_size += host_packed_total_size;
      }
    }
  }
  ...
  bool separate_base_and_mips_descriptors =
      texture_resolution_scale > 1 && mip_first == 0 && mip_last != 0;
  // Destination.
  ...
  // The loop is slices within mips because the base and the mips may need
  ...
  for (uint32_t loop_mip = loop_mip_first; loop_mip <= loop_mip_last;
    ...
    load_constants.guest_base += texture->mip_offsets[mip];
    uint32_t guest_slice_size = texture->slice_sizes[mip];
    ...
    if (texture_resolution_scale > 1 &&
      ...
      uint32_t source_page =
          is_base ? texture->key.base_page : texture->key.mip_page;
      uint32_t source_size_unscaled =
          is_base ? texture->base_size : texture->mip_size;
      if (!MakeScaledResolveRangeCurrent(source_page << 12,
                                         source_size_unscaled)) {
    ...
    const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& host_slice_0_layout =
        mip == mip_packed ? host_packed_slice_0_layout
                          : host_unpacked_slice_0_layouts[mip - mip_first];
    uint32_t host_slice_size = uint32_t(
        mip == mip_packed ? host_packed_slice_size
                          : host_unpacked_slice_sizes[mip - mip_first]);
    load_constants.host_base = uint32_t(host_slice_0_layout.Offset);
    if (mip_packed == 0 && loop_mip) {
      // Two packed mip tails, but this one is for the mips.
      load_constants.host_base += uint32_t(host_packed_mips_offset);
    }
    load_constants.host_pitch = host_slice_0_layout.Footprint.RowPitch;
    ...
    if (mip == 0) {
      ...
      load_constants.guest_storage_width_height[0] =
          xe::align(xe::next_pow2(load_constants.size_blocks[0]), uint32_t(32));
      load_constants.guest_storage_width_height[1] =
          xe::align(xe::next_pow2(load_constants.size_blocks[1]), uint32_t(32));
    }
    ...
    for (uint32_t slice = 0; slice < slice_count; ++slice) {
      ...
      load_constants.guest_base += guest_slice_size;
      load_constants.host_base += host_slice_size;
    }
  }
  ...
  for (uint32_t mip = mip_first; mip <= mip_last; ++mip) {
    ...
    if (mip >= mip_packed) {
      location_source.PlacedFootprint = host_packed_slice_0_layout;
      if (mip != 0) {
        location_source.PlacedFootprint.Offset += host_packed_mips_offset;
      }
      host_slice_size = host_packed_slice_size;
      uint32_t mip_offset_blocks_x, mip_offset_blocks_y, mip_offset_z;
      texture_util::GetPackedMipOffset(width, height, depth, guest_format, mip,
                                       mip_offset_blocks_x, mip_offset_blocks_y,
                                       mip_offset_z);
      ...
      source_box.back = source_box.front + std::max(depth >> mip, uint32_t(1));
      source_box_ptr = &source_box;
    } else {
      uint32_t host_layout_mip_index = mip - mip_first;
      location_source.PlacedFootprint =
          host_unpacked_slice_0_layouts[host_layout_mip_index];
      host_slice_size = host_unpacked_slice_sizes[host_layout_mip_index];
      source_box_ptr = nullptr;
    }
    for (uint32_t slice = 0; slice < slice_count; ++slice) {
      command_list.D3DCopyTextureRegion(&location_dest, 0, 0, 0,
                                        &location_source, source_box_ptr);
      location_dest.SubresourceIndex += texture_mip_count;
