...
DEFINE_transient_path(config_file, "", "Path of xenia's config file",
                      "General");
...
void Config::SetupConfig(const std::filesystem::path& config_folder) {
  ...
  if (!cvars::config_file.empty()) {
    if (std::filesystem::exists(cvars::config_file)) {
      config_path_ = cvars::config_file;
      config_folder_ = config_path_.parent_path();
