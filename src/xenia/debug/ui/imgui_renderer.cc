...
ImGuiRenderer* ImGuiRenderer::ImGuiRenderer::global_renderer_ = nullptr;

ImGuiRenderer::ImGuiRenderer(xe::ui::Window* window,
                             xe::ui::GraphicsContext* context)
    : window_(window),
      context_(context),
      vertex_buffer_(kMaxDrawVertices * sizeof(ImDrawVert)),
      index_buffer_(kMaxDrawIndices * sizeof(ImDrawIdx)) {
  assert_null(global_renderer_);
  global_renderer_ = this;
...
ImGuiRenderer::~ImGuiRenderer() {
  ...
  assert_true(global_renderer_ == this);
  global_renderer_ = nullptr;
}
...
void ImGuiRenderer::Initialize() {
  ...
  io.RenderDrawListsFn = [](ImDrawData* data) {
    global_renderer_->RenderDrawLists(data);
  };
...
void ImGuiRenderer::InitializeShaders() {
...
void main() {
  out_color = vtx_color * texture(texture_sampler, vtx_uv);
})";
...
void ImGuiRenderer::InitializeFontTextures() {
  auto& io = ImGui::GetIO();
  ...
  glTextureParameteri(font_texture, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  glTextureParameteri(font_texture, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTextureStorage2D(font_texture, 1, GL_RGBA8, width, height);
  glTextureSubImage2D(font_texture, 0, 0, 0, width, height, GL_RGBA,
                      GL_UNSIGNED_BYTE, pixels);
  io.Fonts->TexID =
      reinterpret_cast<void*>(static_cast<intptr_t>(font_texture));
}

void ImGuiRenderer::Shutdown() {
  xe::ui::GraphicsContextLock lock(context_);
  ...
  auto tex_id =
      static_cast<GLuint>(reinterpret_cast<intptr_t>(io.Fonts->TexID));
  glDeleteTextures(1, &tex_id);

  ImGui::Shutdown();
}

void ImGuiRenderer::RenderDrawLists(ImDrawData* data) {
  // Setup render state:
  ...
  // Run through and upload all the vertex and index data.
  auto vertex_alloc =
      vertex_buffer_.Acquire(data->TotalVtxCount * sizeof(ImDrawVert));
  auto index_alloc =
      index_buffer_.Acquire(data->TotalIdxCount * sizeof(ImDrawIdx));
  uintptr_t vertex_offset = 0;
  uintptr_t index_offset = 0;
  for (int i = 0; i < data->CmdListsCount; ++i) {
    auto cmd_list = data->CmdLists[i];
    size_t vertex_bytes = cmd_list->VtxBuffer.size() * sizeof(ImDrawVert);
    std::memcpy(
        reinterpret_cast<uint8_t*>(vertex_alloc.host_ptr) + vertex_offset,
        cmd_list->VtxBuffer.Data, vertex_bytes);
    vertex_offset += vertex_bytes;
    size_t index_bytes = cmd_list->IdxBuffer.size() * sizeof(ImDrawIdx);
    std::memcpy(reinterpret_cast<uint8_t*>(index_alloc.host_ptr) + index_offset,
                cmd_list->IdxBuffer.Data, index_bytes);
    index_offset += index_bytes;
  }
  vertex_buffer_.Commit(std::move(vertex_alloc));
  vertex_buffer_.Flush();
  index_buffer_.Commit(std::move(index_alloc));
  index_buffer_.Flush();

  // Second pass to issue draws.
  ...
  for (int i = 0; i < data->CmdListsCount; ++i) {
    ...
    for (int j = 0; j < cmd_list->CmdBuffer.size(); ++j) {
      ...
      if (cmd.TextureId != prev_texture_id) {
        glBindTextureUnit(
            0, static_cast<GLuint>(reinterpret_cast<intptr_t>(cmd.TextureId)));
        prev_texture_id = cmd.TextureId;
      }
      glScissorIndexed(0, static_cast<int>(cmd.ClipRect.x),
                       static_cast<int>(height - cmd.ClipRect.w),
